...
//		   String hql = "SELECT b FROM Book b JOIN b.shelf.id s WHERE s.room.id = :roomId";
	...
	public List<Borrower> getOverdueBorrowers() {
	    try (Session session = HIbernateConfig.getSession().openSession()) {
	        String hql = "FROM Borrower WHERE returnDate > dueDate"; // Assuming overdue if return date is after due date
	        TypedQuery<Borrower> query = session.createQuery(hql, Borrower.class);
